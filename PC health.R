library(stringr)
july9data<-read.csv("2016-07-13123041Shipment_Closed.csv",header=T,stringsAsFactors = F)
...
july9data <- july9data %>% ##converting the format of date and time into required format
mutate(IN_BAG_TIMESTAMP = as.POSIXct(strptime(IN_BAG_TIMESTAMP,"%d-%m-%Y %H:%M:%S")), BAG_CLOSED_TIMESTAMP = as.POSIXct(strptime(july9data$BAG_CLOSED_TIMESTAMP,"%d-%m-%Y %H:%M:%S")) )
...
july9data_cumulative<- july9data_sorted
july9data_sorted<- july9data_sorted[order(july9data_sorted$hour),]
july9data_sorted$cum<- cumsum(july9data_sorted$Package_sorted)

july9data_sorted<-subset(july9data_sorted,!is.na(july9data_sorted$hour) & july9data_sorted$hour != "")

p4<-plot_ly(july9data_sorted,x=hour,y=cum)
p4

july9data_bagclosed$cum2<- cumsum(july9data_bagclosed$Package_bagged)
p5<- plot_ly(july9data_bagclosed,x=hourbagged,y=cum2)
p5
p4 %>% add_trace(p5)

july9data_bagclosed$hour<-july9data_bagclosed$hourbagged
plot_ly(july9data_sorted,x=hour,y=july9data_sorted$cum) %>% add_trace(july9data_bagclosed,x=hour,y=july9data_bagclosed$cum2)
